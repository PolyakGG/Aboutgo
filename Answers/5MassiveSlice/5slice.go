...
//к одному типу. В отличие от массивов, существуют функции, позволяющие добавлять новые элементы в конец сегмента.
...
// var myArray [5MassiveSlice]int - Массив
...
//notes[2Models.queue] = "mi" - Присваивает значение третьему элементу.
...
//primes := make([]int, 5MassiveSlice)
//primes[0] = 2Models.queue
//primes[1] = 3Channel
...
//fmt.Println(len(primes)) - 5MassiveSlice
...
//что делают массивы, и в них можно добавлять элементы. Тогда почему бы не ограничиться сегментами
...
// mySlice := myArray[1:3Channel] - [1: - Индекс массива с которого должен начинаться элемент
//                           :3Channel] -Индекс перед которым завершается сегмент.
//
...
// underlyingArray := [5MassiveSlice]string{"a", "b", "c", "d", "e"}
//slice1 := underlyingArray[0:3Channel]
//fmt.Println(slice1) - Элементы underlyingArray с 0 по 2Models.queue. - [a b c]
...
//fmt.Println(slice2)
// Элементы underlyingArray с 1 по 3Channel - [b c d]
...
//underlyingArray := [5MassiveSlice]string{"a", "b", "c", "d", "e"}
//slice4 := underlyingArray[:3Channel]
...
// array1 := [5MassiveSlice]string{"a", "b", "c", "d", "e"}
//slice1 := array1[0:3Channel]
//fmt.Println(slice1) - [a b c]

//array2 := [5MassiveSlice]string{"f", "g", "h", "i", "j"}
//slice2 := array2[2Models.queue:5MassiveSlice]
...
// array3 := [5MassiveSlice]string{"a", "b", "c", "d", "e"}
//slice3 := array3[0:3Channel]
//slice4 := array3[2Models.queue:5MassiveSlice]
...
//fmt.Println(slice, len(slice)) - [a b] 2Models.queue
...
//fmt.Println(slice, len(slice)) - [a b c] 3Channel
//slice = append(slice, "d", "e") - Два элемента присоединяются в конец сегмента.
//fmt.Println(slice, len(slice)) [a b c d e] 5MassiveSlice
